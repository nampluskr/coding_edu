...
/*
[ver4]
 ver3 + 그룹 크기 작은 순서로 처리

 seed=5,  SCORE: 3001673
 seed=32, SCORE: 3011679
*/
#include <stdio.h>
...
int gids[10000], sorted[10000], gcnt;	// 그룹 번호 리스트, 크기 작은순으로 정렬된 그룹 번호 리스트, 그룹 개수
...
void countingSort() {
	...
	for (int i = 0; i < gcnt; i++) C[gn[gids[i]]]++;
	for (int i = 1; i < 30; i++) C[i] += C[i - 1];
	for (int i = gcnt - 1; i >= 0; i--) sorted[--C[gn[gids[i]]]] = gids[i];
}
